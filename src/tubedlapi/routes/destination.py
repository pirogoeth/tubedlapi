...
blueprint = Blueprint("destination", __name__, url_prefix="/destinations")
log = logging.getLogger(__name__)


@blueprint.route("/", methods=["GET"])
def list_destinations() -> Response:
    """ Returns a JSON list of all destinations with secrets sanitized out of
        the `config` property.
...
def create_destination() -> Response:
    ...
    if not name:
        return (
            jsonify({"message": "name required but not provided"}),
            status.BAD_REQUEST,
        )

    url = payload.get("url")
    if not url:
        return jsonify({"message": "url required but not provided"}), status.BAD_REQUEST

    dest = Destination(**payload)
    try:
        dest.save()
        return jsonify({"message": "created", "destination": dest.to_dict()})
    except peewee.IntegrityError:
        return jsonify({"message": "name already exists"}), status.CONFLICT
...
def delete_destination(name) -> Response:
    ...
    try:
        ...
        return jsonify({"message": "deleted", "destination": last_state})
