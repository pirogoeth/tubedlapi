...
class Settings(Component):
    ...
    def init(cls: Type[Component]) -> "Settings":
        ...
        try:
            this.CRYPTO_KDF_ITERATIONS = int(
                os.getenv("CRYPTO_KDF_ITERATIONS", Settings.CRYPTO_KDF_ITERATIONS)
            )
        except ValueError:
            log.exception(
                "env:CRYPTO_KDF_ITERATIONS is not an integer, default to 10000"
            )
            this.CRYPTO_KDF_ITERATIONS = Settings.CRYPTO_KDF_ITERATIONS

        # Sentry settings
        this.SENTRY_LOG_LEVEL = logging._nameToLevel.get(
            os.getenv("SENTRY_LOG_LEVEL", "WARNING").upper(), logging.WARNING
        )
...
component = {"cls": Settings, "init": Settings.init, "persist": True}
